...
def read_protein(filepath,prot_lm):
    ...
    pro_seq_emb = Data(seq = None)

    return pocket_coord, atom_fea,protein_pocket,h_num,pro_seq_emb, env_seq
...
def Mult_graph(lig_file_name, pocket_file_name, id, score,port_lm):
    ...
    pocket_coord, pocket_atom_fea,protein,h_num_pro,pro_seq, env_seq= read_protein(pocket_file_name,port_lm)  #,pro_seq
...
def Inter_graph(lig_coord, pocket_coord, lig_atom_fea, pocket_atom_fea, env_seq, lig_file_name, score, cut=5):
    ...
    # poc_edge_index, poc_edge_attr = cal_edge(pocket_coord, threshold=10.0)
    # data['protein', 'protein'].edge_index = poc_edge_index
    # data['protein', 'protein'].edge_attr = poc_edge_attr
    ...
    # data['env', 'protein'].edge_attr = torch.full((env2p_edge_index.shape[1], 1), 0.1, dtype=torch.float32)
    # data['env', 'ligand'].edge_attr = torch.full((env2l_edge_index.shape[1], 1), 0.1, dtype=torch.float32)
...
def process_raw_data(dataset_path, processed_file, protein_list):
    ...
    # model = T5EncoderModel.from_pretrained("ProstT5")
...
if __name__ == '__main__':
    ...
    raw_data_path = './data/PdbBind/core-set/'
    protein_list = GetPDBList(Path='./data/PdbBind/index/INDEX_general_PL_data.2016')

    process_raw_data(raw_data_path, './data/core-set.pkl', protein_list)
