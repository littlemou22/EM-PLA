...
class CAPLA(nn.Module):
    def __init__(self, seq_input_size, smi_input_size, out_dim, mode):
        ...
        self.env_embed = nn.Linear(5, smi_embed_size)
        ...
        self.pkt_attention_env = EncoderLayer(128, 128, 0.1, 0.1, 2)
        self.smi_attention_env = EncoderLayer(128, 128, 0.1, 0.1, 2)
    ...
    def forward(self, seq, pkt, coords, env):
        ...
        env_embed = self.env_embed(env)

        smi_embed = self.smi_attention_env(smi_embed, env_embed)
        pkt_embed = self.pkt_attention_env(pkt_embed, env_embed)
